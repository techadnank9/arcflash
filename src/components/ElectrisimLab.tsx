...
const DEMO_CHECKPOINTS = [
  ...
  {
    id: 'palette-items',
    label: 'Locate the exact palette symbols',
    detail: 'Use Generator (~), the first Transformer, two External Grids, Motor (M), and Bus.',
    match: /(?:generator|source|tilde|~)[^.]{0,140}transformer[^.]{0,140}external grid[^.]{0,140}(?:motor|rotating)[^.]{0,140}bus/i,
  },
  {
    id: 'components-placed',
    label: 'Place all five components up top',
    detail: 'Align Generator, Transformer, two External Grids, and Motor in the upper third.',
    match: /(?:plac(?:e|ed)|align(?:ed)?|drag(?:ged)?)[^.]{0,120}generator[^.]{0,120}transformer[^.]{0,140}(?:two )?external grid[^.]{0,140}motor/i,
  },
  {
    id: 'bus-connected',
    label: 'Connect the topology with Bus',
    detail: 'Snap Bus conductors across Generator, Transformer, both External Grids, and Motor.',
    match: /(?:bus|conductor)[^.]{0,100}(?:connect|snap|join)[^.]{0,160}(?:generator|transformer|external grid|motor)|(?:connect|snap|join)[^.]{0,100}(?:bus|conductor)/i,
  },
  {
    id: 'visual-confirmation',
    label: 'Confirm the upper centered diagram',
    detail: 'Verify Generator — Transformer — External Grid — External Grid — Motor above the midpoint.',
    match: /(?:upper third|above[^.]{0,30}midpoint)[^.]{0,160}(?:center|generator|transformer|external grid|motor)|(?:confirm|visible)[^.]{0,140}generator[^.]{0,100}transformer[^.]{0,140}external grid[^.]{0,140}motor/i,
    requiresObservation: true,
  },
  {
    id: 'safe-stop',
    label: 'Stop without saving or simulation',
    detail: 'No login, component configuration, simulation, export, save, or connected storage.',
        ...
        <section className="electrisim-workspace" aria-label="Electrisim browser demonstration controls">
          <aside className="electrisim-safety">
            ...
            <dl>
              ...
              <div><dt>Required tool</dt><dd>Atomic drag_web; never click_web + move_mouse_web</dd></div>
